        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-16">
            ...
            <nav className="hidden md:flex items-center space-x-8">
              {navLinks.map((link) => (
                link.isRoute ? (
                  <Link key={link.name} to={link.isHome ? link.href : `/${link.href}`} className="text-stone-600 hover:text-forest-600 transition-colors">
                    {link.name}
                  </Link>
                ) : (
                  <a
              ...
              {user ? (
                ...
                <div className="flex items-center space-x-4">
                  {/* Sign In commented out per user request */}
                  {/* <Link
                    to="/login"
                    className="text-stone-600 hover:text-forest-600 transition-colors font-medium"
                  >
